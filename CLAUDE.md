...
infrastructure). There is no `app/`, no root `database/`, no root `config/` —
`osdd:start` removed them. Use the `osdd:*` artisan commands, which target a
layer, instead of the `make:*` ones.
